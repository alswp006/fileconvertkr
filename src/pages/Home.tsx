 ...
 *   FileConvertKR -> the app's display name
 *   아이폰 사진이 카톡에서 안 열려요? HEIC→JPG, PDF 합치기·압축 3초 변환    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>FileConvertKR</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="FileConvertKR"
        value={<Paragraph.Text typography="t2">아이폰 사진이 카톡에서 안 열려요? HEIC→JPG, PDF 합치기·압축 3초 변환</Paragraph.Text>}
